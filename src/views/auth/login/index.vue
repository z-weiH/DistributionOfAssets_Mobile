    ...
    <div class="main_wrap">
      <flexbox class="info_box">
        ...
        <flexbox-item :span="7" class="wrap_input">
          <x-input name="loginName" type="tel" v-validator="validator.loginName" v-model="loginName" placeholder="请输入您的手机号码" style="font-size:15px"  mask="999 9999 9999" :max="13" ></x-input>
        </flexbox-item>
        <flexbox-item :span="4" class="error_holder">
          <span>{{$validator.firstError('loginName')}}</span>   <!-- 手动添加错误提示 -->
        </flexbox-item>
...
export default {
	...
	methods: {
		...
		bindUserQuery() {
			// 查询是否绑定账号
			this.$api.get('/mobile/band/user/query.htm?openId=' + this.openId).then(res => {
				if (res.data.code === '0000') {
					if (!res.data.result) {
						// 未绑定
						this.bindType = true
