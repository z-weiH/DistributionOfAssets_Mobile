<template>
 <div class="page">
    <flexbox class="info_box">
      ...
      <flexbox-item>
        <x-input name="newPwd" type="password" v-model="newPwd" placeholder="请输入新密码(6位或以上的数字或字母)"></x-input>
      </flexbox-item>
    </flexbox>
    <flexbox class="info_box">
      <flexbox-item :span="1">
      ...
      <flexbox-item>
        <x-input name="confirmPwd" type="password" v-model="confirmPwd" placeholder="请再次输入密码"></x-input>
      </flexbox-item>
    </flexbox>
...
export default {
	...
	methods: {
		getParams() {
			...
			this.validCode = this.$route.params.validCode
		},
		postNewInfoFoo() {
			let _obj = {
        ...
        validCode:this.validCode
			...
			} else if (this.newPwd.length != this.newPwd.length) {
				this.$vux.toast.text('两次输入的密码不一致')
			} else {
				this.$api.get(`/mobile/password/set.htm?${qs.stringify(_obj)}`).then(res => {
